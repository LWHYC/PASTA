...
from dataloaders.get_loader import get_classify_loader
...
def train(args, rank, world_size):
    ...
    # ========== 1. Build logger and TensorBoard writer ==========
    ...
    # ========== 2. Random seed and CUDNN setting ==========
    ...
    if args.pretrained_model_path is not None:
        net = load_pretrained(args, net, optimizer, logger)

    # ========== 7. Create loss function ==========
    if isinstance(class_num, list):
        loss_fn = MultiClassCrossEntropy_Loss(multiclass_num=class_num)
    else:
        loss_fn = MultiClassCrossEntropy_Loss(multiclass_num=[class_num])

    # ========== 8. Load data ==========
    logger.print('3. Load data')
    trainData, validData = get_classify_loader(args)
                ...
                if train_step % save_step == 0:
                    ...
                    with torch.no_grad():
                        for j_batch, valid_sample in enumerate(validLoader):
                            ...
                            valid_label = valid_sample['label'].cuda(rank)
                            ...
                            for group_i, (start, end) in enumerate(class_num_index):
                                ...
                                group_label = valid_label[:, start:end]         # shape: (B, group_class_num), one-hot
                        ...
                        if rank == 0:
                            ...
                            filename = f"{model_save_name}_cur_Acc{overall_acc:.4f}_{train_step}.tar"
                            if overall_acc > best_acc:
                                best_acc = overall_acc
                                is_best = True
                                bestname = f"{model_save_name}_best_Acc{overall_acc:.4f}_{train_step}.tar"
                            else:
                                is_best = False
                                bestname = None
